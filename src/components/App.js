...
class App extends Component {
  ...
  handleCommandSent({syncer, command, subscribe, getResponse}) {
    ...
    subscribe(({line, response, end}) => {
      ...
      if (
        line.slice(0, 5) === 'info ' &&
        this.state.treePosition === treePosition
      ) {
        let tree = this.state.gameTrees[this.state.gameIndex]
        ...
        let analysis = line
          ...
          .map(x => {
            ...
            if (matchPass == null) {
              ...
              return [
                x.slice(0, matchPV.index),
                matchPV[0].slice(0, matchPass.index).split(/\s+/)
              ]
            }
          })
          .filter(x => x != null)
  ...
  async startAnalysis({showWarning = true} = {}) {
    ...
    this.setState({
      analysis: null,
      analysisTreePosition: this.state.treePosition
    })
    ...
    try {
      ...
      let engineIndex = controllerIndices.find(
        i =>
          this.attachedEngineSyncers[i] != null &&
    ...
    if (showWarning && error) {
      dialog.showMessageBox(
        t('You haven’t attached any engines that supports analysis.'),
        'warning'
      )
      this.stopAnalysis()
    }
